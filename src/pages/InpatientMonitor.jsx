...
import { Activity, AlertTriangle, RefreshCw, ShieldCheck, Search, BedDouble } from "lucide-react";
...
export default function InpatientMonitor() {
  ...
  const [filter, setFilter] = useState("all"); // all, critical, warning, stable
  const [lastUpdated, setLastUpdated] = useState(null);

  useEffect(() => {
    loadInpatients();
    const interval = setInterval(loadInpatients, 60000); // auto-refresh every 60s
    return () => clearInterval(interval);
  }, []);

  const loadInpatients = async () => {
    try {
      const data = await base44.entities.CaseFile.filter(
        { status: "admitted" },
        "-updated_date",
        100
      );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={loading}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-secondary text-secondary-foreground text-xs font-medium hover:bg-secondary/80"
          >
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-card border border-border rounded-lg pl-9 pr-3 py-2 text-sm text-foreground focus:outline-none focus:border-hive-gold/50"
          />
        </div>
          ...
          {[
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${
                filter === f.key
                  ? "bg-hive-gold/15 text-hive-gold"
      ...
      {loading ? (
        ...
        <div className="bg-card border border-border rounded-xl p-12 text-center">
          ...
          <p className="text-muted-foreground text-sm">
            {cases.length === 0
              ? "No admitted patients currently being monitored."
              : "No patients match your filter."}
          </p>
          {cases.length === 0 && (
            <Link
              to="/new-referral"
              className="inline-flex items-center gap-2 mt-4 px-4 py-2 rounded-lg bg-hive-gold text-hive-gold-foreground font-medium text-sm hover:bg-hive-gold/90"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {filteredCases.map(c => (
            <InpatientCard key={c.id} caseFile={c} />
          ))}
        </div>
      )}
    </div>
