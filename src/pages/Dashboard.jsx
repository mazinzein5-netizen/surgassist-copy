...
import { FilePlus2, FolderOpen, AlertTriangle, ClipboardList, Users, Calculator, Activity, Clock, ChevronRight } from "lucide-react";
...
export default function Dashboard() {
  ...
  const loadCases = async () => {
    try {
      const data = await base44.entities.CaseFile.filter({}, "-created_date", 10);
      setCases(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const stats = [
    ...
    { label: "Pending Review", value: pendingReview.length, icon: Clock, color: "text-warning", to: "/cases" },
    { label: "Today's Referrals", value: cases.length, icon: Activity, color: "text-hive-gold", to: "/cases" },
    { label: "Theatre Log", value: "—", icon: ClipboardList, color: "text-success", to: "/theatre-log" },
  ];
